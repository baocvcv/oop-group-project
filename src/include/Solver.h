...
public:
    ...
    void save_solver(const std::string& filename);
    void save_solution(const std::string& filename);

   // return matrix[time][m][n]
    std::vector<std::vector<std::vector<std::pair<int, int>>>> get_grid();

    // return dispensers[p] (pos, label)
    std::vector<int> get_sink_dispenser_pos();
...
inline void Solver::save_solver(const std::string& filename){
